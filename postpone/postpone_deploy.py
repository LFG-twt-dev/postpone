...
def set_lambda_rule(function_arn, rule_name):
    target_details = subprocess.run(
        f'aws events put-targets --rule {rule_name} --targets "Id"={uuid.uuid4()},"Arn"="{function_arn}" --output json', capture_output=True, shell=True)
...
def setup_trigger(function_name, rule_name):
    ...
    result = subprocess.run(
        f'aws lambda add-permission --function-name "{function_name}" --action "lambda:InvokeFunction" --principal events.amazonaws.com --statement-id MyEvent --source-arn $(aws events describe-rule --name "{rule_name}"  --query "Arn" --output "text") --output "json"', capture_output=True, text=True, shell=True)
...
def postpone_deploy(file_name, handler, runtime, minutes):
    ...
    role_data = subprocess.run(
        f"aws iam create-role --role-name {role_uuid} --assume-role-policy-document '{policy_string}'", shell=True, text=True, capture_output=True)
    role_data_json = json.loads(role_data.stdout)
    role_arn = role_data_json["Role"]["Arn"]
    time.sleep(3)
    subprocess.run(
        f"aws iam attach-role-policy --role-name {role_uuid}  --policy-arn arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole", shell=True, text=True, capture_output=True)
    subprocess.run(['zip', "./lambda.zip", "./"+file_name+".py"], check=True)
    time.sleep(3)
    function_name = str(uuid.uuid4()).replace('-', '')
    function_arn = create_lambda_function(
        function_name, "lambda.zip", handler, runtime, role_arn)
    rate_unit = "minute"
    if (int(minutes) > 1):
        rate_unit = "minutes"
    if (int(minutes) < 0):
    ...
    subprocess.run(
        f'aws events put-rule --schedule-expression "rate({minutes} {rate_unit})" --name {rule_name}', text=True, capture_output=True, shell=True)
    time.sleep(3)
